...
def impainting():
    ...
    for idx in range(num_batches):
        ...
        # 先保存一下原始图片和masked图片
        save_tensor_images(batch_images.detach(),os.path.join(source_imagedir,"source_image_batch_{}.png".format(idx)))
        save_tensor_images(masked_batch_images.detach(), os.path.join(masked_imagedir, "masked_image_batch_{}.png".format(idx)))

       

        z_hat = torch.rand(low=-1.0,high=1.0,[realBatchSize,z_dim],require_grad=True)
        ...
        for iteration in range(args.num_iters):
            ...
            batch_images_g_masked = torch.mul(batch_images_g,1-mask).to(device)
            impainting_images = (batch_images_g_masked+masked_batch_images).to(device)

            loss_context = torch.norm(
                (batch_images_g_masked-torch.mul(batch_images_g,mask).to(device)),p=1)
            dis_output = discriminator(impainting_images)
            batch_labels = torch.full((realBatchSize,), 1, device=device)
            loss_perceptual = criteria(batch_labels,dis_output)

            total_loss = loss_context + lamd*loss_perceptual
            print("\rbatch {} : iteration : {:4} , context_loss:{:.4f},percptual_loss:{:4f}".format(idx,iteration,loss_context,loss_perceptual),end="")
            total_loss.back_ward()
            opt.step()
            z_hat = torch.clamp(z_hat,low=-1.0,high=1.0).to(device)

            if iteration % 100==0:
                # 保存impainting 图片结果
                print("\nsaving impainted images for batch: {} , iteration:{}".format(idx,iteration))
                save_tensor_images(impainting_images, os.path.join(impainted_imagedir,"impainted_image_batch_{}_iteration_{}.png".format(idx,iteration)))
